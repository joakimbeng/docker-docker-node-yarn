...
# mhart/alpine-node:10.12.0 slightly modified (added bash, curl, git and openssh)
ENV VERSION=v10.12.0 NPM_VERSION=6 YARN_VERSION=latest

# For base builds
# ENV CONFIG_FLAGS="--fully-static --without-npm" DEL_PKGS="libstdc++" RM_DIRS=/usr/include

RUN apk add --no-cache bash openssh git curl make gcc g++ python linux-headers binutils-gold gnupg libstdc++ && \
