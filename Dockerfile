...
# mhart/alpine-node:9.4.0 slightly modified (added PATH env and installing bash, git and openssh)
ENV VERSION=v9.4.0 NPM_VERSION=5 YARN_VERSION=latest PATH=node_modules/.bin:$PATH

# For base builds
# ENV CONFIG_FLAGS="--fully-static --without-npm" DEL_PKGS="libstdc++" RM_DIRS=/usr/include

RUN apk add --no-cache bash git openssh curl make gcc g++ python linux-headers binutils-gold gnupg libstdc++ && \
